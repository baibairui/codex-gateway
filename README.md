...
- 默认走 `observe -> act -> observe`：先截图观察，再执行一个 2-5 步的小型 GUI 动作包，然后立刻补证据。
- 单个动作包只能覆盖同一个明显焦点链路，不应在中途依赖新的界面判断。
...
1. 先用 `observe` 获取当前界面截图和前台应用信息。
2. 当下一步足够明确时，再用 `act` 执行一个 2-5 步的 GUI 动作包。
3. 动作包完成后立刻再次 `observe`，用新截图确认结果。
4. 按 `Action Bundle / Evidence / Result / Next step` 回报。
...
桌面截图默认保存在当前工作区的 `./.codex/artifacts/desktop/` 下，便于 agent 回报证据和继续引用。
