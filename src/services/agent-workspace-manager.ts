...
function renderBrowserPlaybook(): string {
  return [
    ...
    '',
    '## Execution Mode',
    '- 浏览器任务优先通过 skill 执行（如 `playwright-explore-website`）。',
    '- 不依赖网关注入 Playwright MCP；由 skill 负责浏览器交互流程。',
    '- 优先复用已登录会话，减少重复登录和重复授权。',
    '',
    '## Default Workflow',
    '1. 明确目标、约束和验收标准后再操作。',
    '2. 先读取页面状态（快照/结构化信息），再执行动作。',
    '3. 小步执行：点击、输入、选择，每步可回退。',
    '4. 每步后汇报：做了什么、看到了什么、下一步做什么。',
    '',
    '## Guardrails',
    '- 无可见证据时不得宣称完成。',
    '- 遇到登录、OTP、验证码、支付确认，必须请求用户接管并等待。',
    '- 人工接管完成后，要求用户明确回复“继续”，再恢复后续步骤。',
    '- 对外回复不暴露内部路径、文件名、实现细节。',
    '',
    '## Output Style',
    '- 更新简洁明确：动作 -> 结果 -> 下一步。',
    '',
